...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if  (real_close_point(@x, @y, seg.x1, seg.y1) || real_close_point(@x, @y, seg.x2, seg.y2)) || ((@x >= seg.x1) && (@x <= seg.x2) && (@y >= seg.y1) && (@y <= seg.y2))
      Point.new(@x, @y)
    else
      NoPoints.new()
    end
...
class Let < GeometryExpression
  ...
  def eval_prog env
    #from sml: eval_prog (e2, ((s, eval_prog(e1,env)) :: env))
    pr = env.assoc @s
    if pr.nil?
      @e2.eval_prog(env.append([@s, @e1.eval_prog(env)]))
    else
      @e2.eval_prog(env.map{|x| x[0] == @s ? [@s,@e1.eval_prog(env)] : x})
    end
  end
  def preprocess_prog
...
class Shift < GeometryExpression
  ...
  def preprocess_prog
    Shift.new(@dx, @dy, @e.preprocess_prog) # no pre-processing to do here
  end
  def shift(dx,dy)
    self
  end
